...
class UAV():
	...
	def arm(self,bool):
		...
		wp.command = 400  # MAVROS Arm/Disarm
	...
	def takeOff(self):
		...
		wp.frame = 2  # Global frame:0 with relative alt: 3
	...
	def processOrder(self):
		...
		if order.idAgent != self.id:
			...
			if order.order_name == 1:
				...
				rospy.loginfo("%s:NET_ACTION" % rospy.get_time())
				self.NET_ACTION()
				processOrderFlag = True
				print("NT")
...
def GNC_Node_Action():
	...
	uav.setHomeLocation(True, [-35.362938, 149.165085, 584.0]) # original
	#uav.setHomeLocation(False, [0, 0, 0])
	#uav.setFlightMode(mode='AUTO') #uncomment out when AUTO bug is found
	uav.arm(1)
	...
	uav.waypoint([20, 20, 10])
	uav.waypoint([20, 0, 10])
	uav.waypoint([20, 30, 10])
